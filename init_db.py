...
def init_db():
    """
    Initializes a fresh database with the correct schema, 
    a default admin user, and sample questions.
    """
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    ...
    # --- Create Tables with the correct, final schema ---
    ...
    CREATE TABLE user_progress (
        ...
        last_played_on DATETIME NOT NULL,
        FOREIGN KEY (user_id) REFERENCES users (id)
    );
    ''')
    ...
    if not cursor.fetchone():
        ...
        # Correctly inserts into 'hashed_password'
        cursor.execute(
            "INSERT INTO users (full_name, email, hashed_password, is_admin) VALUES (?, ?, ?, ?)",
            ("Admin Shujaa", admin_email, generate_password_hash("Admin@123"), 1)
        )
        print("Default admin user created successfully.")

    # Populate questions table
    sample_questions = [
        # --- Your extensive list of questions goes here ---
        ('phishing', 'You receive an email from "Amaz0n" about a suspicious login. What is the biggest red flag?', 'The link asks you to verify credit card details.', 0, 'The sender\'s email address is slightly misspelled.', 1, 'The email uses the official Amazon logo.', 0, 'It was sent in the middle of the night.', 0, 'Correct! Misspelled sender addresses are a classic phishing tactic.'),
        # ... (include all 80 of your questions)
    ]
...
if __name__ == '__main__':
    # To run this script:
    # 1. Delete the old 'database.db' file.
    # 2. Run 'python init_db.py' in your terminal.
    init_db()